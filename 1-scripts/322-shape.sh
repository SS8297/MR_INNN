...
sweep=$1
params=$(find ../4-params/shape/ -type f | sort)
batches=$(find ../2-results/000-batches/hyper/ -type f | sort)
winsze=(4)
binolp=(4)
...
parallel -v --dry-run -j 1 . 122-shape.sh ::: $sweep  ::: ${winsze[@]} ::: ${binolp[@]} ::: $params :::+ ${montages[@]} :::+ ${spectra[@]} ::: $batches
